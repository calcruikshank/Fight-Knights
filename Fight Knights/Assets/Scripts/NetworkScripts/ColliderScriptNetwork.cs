...
public class ColliderScriptNetwork : MonoBehaviour
{
    NetworkPlayerController opponent;
    ...
    {
        opponent = other.transform.parent.GetComponent<NetworkPlayerController>();
        if (opponent != null && collideTimer <= colliderThreshold)
        {
            if (!moreDamageIfStunned || opponent.state != NetworkPlayerController.State.Stunned)
            {
                this.transform.parent.transform.parent.GetComponent<HandleColliderNetwork>().HandleCollision(hitID, damage, opponent);
            }
            if (moreDamageIfStunned && opponent.state == NetworkPlayerController.State.Stunned)
            {
                this.transform.parent.transform.parent.GetComponent<HandleColliderNetwork>().HandleCollision(hitID, stunDamage, opponent);
            }
